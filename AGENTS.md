...
- オンボーディングの×ボタンと折り返し系統の作成(往路と復路で別番線)は
  2026-07-25にユーザーが実機で確認済み
- 残っている実機確認: 高架駅の建設と見た目(テストは通っているがブラウザでは
  未操作)。加えてUI/UX刷新以降のブラウザ実操作(下記「変更→検証」の4項)、
...
```
   結果は`Logs/edittest.xml`の`<test-run ... failed="0" ...>`で確認する(現在169件PASS
   +1件Ignore)。
...
- **パネルの高さは端末pxではなくCanvasScalerの参照単位**(1000×1600、match 0.5)。
  「844pxの画面に823単位のパネルは入らない」と数値だけで判断すると誤る。スケールは
  `sqrt(w/1000 * h/1600)`で、iPhone 17 Proなら約0.47倍、使える高さは約1650単位ある。
  縦画面シートの高さは`UIController`の定数にまとめ、`UiLayoutTests`が実寸換算で
  収まることを守っている(高さを増やす変更をしたらこのテストが判定する)
...
## 走行経路とレールの一致(解決済み・テストで固定)

走行経路と描画レールは中心線を単一の出所にして統一済み。渡り線も、描画
(`RailKit.CrossoverPath`)と同じS字を走行経路が辿る(実測0.000m)。
保証は次の2段のテストで守っている(`RailGeometryTests`):

- `DrawnRailMesh_FollowsTheStoredCentreLine`: 描画されたレールメッシュが、駅が
  保持する中心線どおり軌間の半分だけ離れて敷かれていること
- `TrainLegRunsOnTheStoredCentreLine`: 走行経路がその中心線(渡り線を含む)の上を
  通ること。発着それぞれで渡り線の要/不要の全組み合わせを見る。経路が飛ばない
  ことも区間長で確認する

**走行中を測るならPlaceCars()を呼ぶこと**。`Train.SimTick`は論理状態しか進めず、
車体のTransformは`Bootstrap.RunFrame`が呼ぶ`PlaceCars()`で初めて反映される。
テストやプローブでSimTickだけを回すと**初期配置しか測れない**。
2026-07-26にこれで「列車は常に線路中心ちょうど」「ホーム侵入0件」という誤った結論を出し、
80ケース以上の総当たりを空振りさせた。走行中の挙動を測るループは必ず
`train.SimTick(dt); train.PlaceCars();`の2行を回す。

**「どれかのレールに近い」では不十分**。走行経路の検証は「**正しい番線の上にいるか**」まで
見ること。距離だけを見る判定は、経路が隣の番線の中心線と重なると0.000mで合格してしまう。
2026-07-26の不具合(渡り線を渡る際に構内の経路が±2.3へ潰され、島式・2面4線では
ホームの内側を走っていた)は、まさにこれで長期間見逃されていた。
`CrossoverPathTests.LegThroughACrossover_KeepsThePlatformTrackOffset`が
「ホーム部ではその番線の座標にいること」を直接見ている。

**測り方の注意**(過去に2回間違えた):
- 走行経路と同じ出所どうしを突き合わせると循環し、描画側だけの退行を検出できない。
  上の2段に分けているのはそのため
- 「経路点から最寄りのレール頂点までの距離」で測ってはいけない。レール頂点は
  ホーム区間で数十m間隔しかなく、頂点間の中点で最大25m程度になる。中心線の各点に
  対してはレール頂点が必ず生成されるので、そちらを起点に測る
- レールメッシュにはスロートの渡り線も含まれる。中心線との距離を見る検証は
  ホーム区間に限定する

## 高さは「階(level)」で持つ(高架/将来の地下)

`Station.level`が唯一の高さ指定。0=地上、1=2階、2=3階…で、実高は
`RailDimensions.HeightOfLevel(level) = level * FloorHeight(8m)`。地下を足す場合は
負値へ広げる想定で、計算は符号を選ばない形にしてある(UIとValidateが0〜3に制限)。

- **駅のメッシュは全てローカル座標**なので、`transform.position.y`を上げるだけで
  駅ごと持ち上がる。`Station.Build()`の冒頭で高さを再確定させるため、位置Yを
  渡し忘れても落ちない
- **縦断は水平形状と分けて解く**。`RailKit.SmoothConnectPath`は水平(XZ)を解いた後、
  `ApplyVerticalProfile`で弧長に沿ったsmoothstepを掛ける。両端の勾配が0になるので
  駅で線路が折れない。**PI法の枝だけでなく`HermitePath`の3つのフォールバックも
  必ずこれを通すこと**(直線水平の経路はフォールバック側へ行くため、片方だけ直すと
  「高低差はつくのに勾配が非対称」という分かりにくい状態になる。実際に一度やった)
- 縦断がS字なので**最大勾配は平均勾配の1.5倍**。上限35‰(`MaxGrade`)を守るのに
  必要な水平距離は`1.5 * Δh / MaxGrade`(Δh=8mなら343m)。この式は新規接続
  (`BuildController.TapTrackStation`)と建て替え(`DescribeGradeViolation`)の両方で使う。
  **建て替え側を忘れると、接続済みの駅の階を上げるだけで急勾配が作れてしまう**
- **立体交差**は高さ判定で許可する。`Station.FootprintsOverlap`/`FootprintContains`/
  `PlatformAreaContains`は、高低差が`LevelClearance`(4m)以上あればfalseを返す。
  これで高架下を線路が通れる(`FindStationCrossedByBed`も自動的に追従する)
- **立体交差を許可したら、重なった駅を選ぶ手段が要る**。上からのレイは高架駅に
  必ず先に当たるので、単純な最寄りヒットだと真下の地上駅が永久に選べない。
  `BuildController.PickStation`が`RaycastAll`で貫いた駅を距離順に集め、同じ場所を
  続けてタップすると順送りする。**高架駅の当たり判定を橋脚ぶん下へ伸ばしてはいけない**
  (下の駅を丸ごと覆ってしまう。一度やってCodexに指摘された)
- 高架構造は視覚層。駅は`Station.AddViaduct`(桁+橋脚、スロート端まで)、駅間は
  `TrackSegment.AddViaduct`(Yが`LevelClearance`以上ある連続区間ごとに桁+橋脚)。
  地面へ降りる勾配部分は盛土扱いで何も足していない
- 費用は`GameState.StationCost(cars, faces, lines, level)`で`1 + 0.6*|level|`倍
- 検証は`Assets/Tests/Editor/ElevatedStationTests.cs`(縦断・勾配上限・走行経路の
  高さ一致・立体交差・建て替え・セーブv5)

**未対応(意図的)**: 地下(地面を掘る表現が別途要る)、勾配上の渡り線
(`RailKit.AddCrossover`は`dir.y=0`前提。駅構内は水平なので当面問題にならない)、
勾配による走行性能の変化、高架下の建物・道路。

## 直近の変更(新しい順)

- 車窓は**ニアクリップを0.05mへ下げる**(地図視点は18km先まで映すため1m)。
  下げないと運転台とモニターが手前すぎて丸ごと切り取られ、何も見えない
- 前面には**窓の開口**がある(以前は黒パネルが窓の範囲まで覆い、その上にガラス板を
  重ねるだけだった)。車窓中はガラスだけ外して枠を残す=窓越しの眺めになる
...
- 渡り線を渡る際の構内経路がホーム内側へ入る不具合を修正。あわせてリード区間の
  収束不足(描かれた渡り線と番線中心線が最大0.34mずれる)も`LeadHold`で解消
- 通過駅を建て替え/撤去したとき、その駅を素通りする列車も経路を作り直すよう修正。
  `RouteHas`は停車駅しか拾わないため、通過列車が古い番線位置のまま新しいホームの
  上を走っていた(`Train.PassesThrough`を追加)
